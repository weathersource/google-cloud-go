...
// both an in-memory API (github.com/weathersource/google-cloud-go/httpreplay) and a standalone server
// (github.com/weathersource/google-cloud-go/httpreplay/cmd/httpr).
package proxy
...
func newProxy(filename string) (*Proxy, error) {
	configOnce.Do(func() {
		...
		// participate in TLS.
		x509c, priv, err := mitm.NewAuthority("github.com/weathersource/google-cloud-go/httpreplay", "HTTPReplay Authority", 100*time.Hour)
		if err != nil {
			configErr = err
			return
		}
		...
		if config != nil {
			...
			config.SetOrganization("github.com/weathersource/google-cloud-go/httpreplay")
